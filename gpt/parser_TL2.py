...
# Original file from https://github.com/hmi-utwente/video-game-text-corpora
# 2020-05-12
import pprint
import re
import json
...
def print_stats_old():
    ...
    for t in [(x['dialog']['intro']['dialog'], x['dialog']['intro']['unit_displayname']) for x in data if
              x.get('dialog') and x['dialog'].get('intro') and x['dialog']['intro'].get('unit_displayname') and x.get(
                      'reward')]:
        print(t)
        print()
...
def print_stats_new():
    ...
    print("Total quests in main questline: {}".format(len([q.get('displayname') for q in quest_data if
                                                           q.get('displayname') and q.get('dialog') and q.get(
                                                               'dialog').get('moredetails')])))
    pprint.pprint([q.get('displayname') for q in quest_data if
                   q.get('displayname') and q.get('dialog') and q.get('dialog').get('moredetails')])
...
def prettify(s):
    ...
    s = re.sub("\n\n", "\n", s)
    s = s.strip()
    s = re.sub(r"\|c[0-9ABCDEF][0-9ABCDEF][0-9ABCDEF][0-9ABCDEF][0-9ABCDEF][0-9ABCDEF][0-9ABCDEF][0-9ABCDEF](.*?)\|u",
               r"\1", s)
    s = re.sub(r"\|u", "", s)
    return s


def print_line(units, line, out):
    ...
    if line.get('unit_displayname'):
        out += f"{line['unit_displayname']}: "
    elif line.get('unitname'):
        out += f"{unit_display_name(units, line['unitname'])}: "
    if line.get('dialog'):
        out += prettify(line['dialog']) + "\n"
    if line.get('dialog_complete'):
        out += prettify(line['dialog_complete']) + "\n"
    return out
        ...
        for dialog_type in quest['dialog'].keys():
            ...
            if type(quest['dialog'][dialog_type]) == list:
                for line in quest['dialog'][dialog_type]:
                    out = print_line(units, line, out)
            else:
                out = print_line(units, quest['dialog'][dialog_type], out)
            out += "\n"
        out += "\n"
    return out
    ...
    for index, line in enumerate(lines):
        ...
        if re.findall(r'^' + '\t' * depth + r'\[([A-Z0-9_]+)\]', line):
            ...
            while not re.findall(r'^' + '\t' * depth + r'\[\/' + tag + r'\]', lines[currentindex]):
                currentindex += 1
            parsed = parsed + lines[
                              startindex - 1: currentindex + 1]  # we do -1 and +1 to include the start and end tag
...
def recurse(questobject):
    ...
    if type(questobject) == dict:
        for k in questobject:
            if k in ['intro', 'passive', 'dialog', 'dialogline', 'complete', 'moredetails', 'return', 'huddetails',
                     'dialog_complete', 'details']:
