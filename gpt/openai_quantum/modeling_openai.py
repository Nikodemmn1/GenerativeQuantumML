...
def q_circuit_function(n_qubits, inputs, w):
...
class Attention(nn.Module):
    def __init__(self, nx, n_positions, config, q_dev, scale=False):
        super().__init__()
        ...
        self.lora_A = nn.Linear(nx, config.lora_rank)

        if q_dev is None:
            self.lora_middle = nn.Linear(config.lora_rank, config.lora_rank)
        else:
            q_circuit = lambda inputs, w: (
                q_circuit_function(config.lora_rank, inputs, w))
            #q_weights_shape = {'random_layer_weights': (1, config.lora_rank * 8)}
            weight_shapes = {'w': (config.str_ent_layers, self.lora_rank, 3)}
            quantum_node = qml.QNode(q_circuit, q_dev, diff_method="best", interface="torch")
            self.lora_middle = qml.qnn.TorchLayer(quantum_node, weight_shapes)

        self.lora_B = nn.Linear(config.lora_rank, n_state * 3)
    ...
    def apply_lora(self, x):
        x = self.lora_A(x)
        x = self.lora_middle(x)
        x = self.lora_B(x)
        return x
    ...
    def forward(self, x, attention_mask=None, head_mask=None, output_attentions=False):
        lora_output = self.apply_lora(x)
        x = self.c_attn(x)
        x += lora_output * (self.lora_alpha / self.lora_rank)
...
class Block(nn.Module):
    def __init__(self, n_positions, config, scale=False, q_dev=None):
        super().__init__()
        nx = config.n_embd
        self.attn = Attention(nx, n_positions, config, q_dev, scale)
...
class OpenAIGPTModel(OpenAIGPTPreTrainedModel):
    def __init__(self, config):
        ...
        if config.lora_quantum:
            q_dev = qml.device('default.qubit',
                               wires=config.lora_rank,
                               c_dtype=np.complex64,
                               r_dtype=np.float32)

            self.h = nn.ModuleList([Block(config.n_positions,
                                          config,
                                          q_dev=q_dev,
