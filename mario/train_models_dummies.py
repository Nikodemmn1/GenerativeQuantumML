...
def train(seq_length, qubits_dummy):
    ...
    model = LSTMArio(vocab_size, hidden_size, num_layers, dropout, quantum_dummy=True, qubits_dummy=qubits_dummy).cuda()
    ...
    for epoch in range(max_epochs):
        ...
        torch.save(model, f'trained_models_dummy/mario_lstm_seqlen={seq_length}_nqubits={qubits_dummy}_'
...
def main():
    for seq_len in [50]:
        for qubit_count in (2, 3, 4, 7):
            train(seq_len, qubit_count)
